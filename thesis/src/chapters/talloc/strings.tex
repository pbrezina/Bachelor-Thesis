...
\begin{funcdesc}
  ...
  the new pointer to |dest|. This may change the pointer address of |dest|, so
  it is unsafe to rely on the old address after calling this function.
\end{funcdesc}
\begin{funcproto}
...
\footnotetext{$2$ for \lstinline{"(|"}, $\sum_{i=1}^m (2 + 8*i + n*i)$ for
