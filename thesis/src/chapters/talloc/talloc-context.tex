...
types and structures. Functions that specialize itselves to strings and arrays
are described later in this text (sections \ref{talloc:sec:strings} and
...
Every talloc context has a name. If we want to create a context with a strong
semantical meaning, we can set a custom name to differentiate it from the other
contexts.
...
checking which is described in section \cmplref{talloc:subsec:type-checking}.
...
\end{funcproto}
\funclistend
The only difference between these two functions is that |talloc_zero()| ensures
that the new memory space is initialized with zeros.
...
void* talloc_prttype(TALLOC_CTX *ctx, #type)
\end{funcproto}
