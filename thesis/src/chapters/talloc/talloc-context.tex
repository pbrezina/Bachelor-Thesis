...
internally. For example we can do this:
...
context using talloc functions, the talloc library will transform the
user-space pointer back to the starting address of the chunk. This is also the
reason why we were unable to use |free(str2)| in the previous example --
because |str2| does not point at the beginning of the allocated block of
memory. This is illustrated on Figure
...
only for semantical reasons -- so we can differentiate between |void*|
...
Every talloc context contains information about its parents and children. Talloc
...
We can divide these functions into four categories: those that allow us to set
...
\begin{itemize}
  \item it returns a new talloc context or |NULL| if the system is out of
  memory,
...
is set to location in the source file where the function is invoked.

If we choose to use these functions we lose both the compile-time and runtime
...
\begin{funcdesc}
  ...
  section \cmplref{talloc:sec:destructors}} attached to this context or one of
  its children fails.
\end{funcdesc}
\begin{funcproto}
...
going to change from the ground\cite{MailListReferences} up. When I asked about
